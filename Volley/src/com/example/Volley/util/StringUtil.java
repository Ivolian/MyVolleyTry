...
public final class StringUtil {
    ...
    public static String[] splitStringByLength(final String text, final int length) {
        ...
        for (int i = 0; i < count; i++) {
            if (i == count - 1) {
                stringArray[i] = StringUtil.substring(text, i * length);
            } else {
                stringArray[i] = StringUtil.substring(text, i * length, length);
            }
        }
    ...
    public static String breakLine(final String text, final int length, final int fetchLineCount) {

        String retValue = "";

        try {
            String[] lineArray = splitStringByLength(text, length);

            int factLineCount = fetchLineCount;
            if (lineArray.length < fetchLineCount) {
                factLineCount = lineArray.length;
            }

            for (int i = 0; i < factLineCount; i++) {
                if (i == factLineCount - 1) {
                    if (lineArray.length > fetchLineCount && length > 3) {
                        retValue += substring(lineArray[i], 0, length - 3) + "...";
                    } else {
                        retValue += lineArray[i];
                    }
                } else {
                    retValue += lineArray[i] + "\r\n";
                }
    ...
    public static void main(String[] args) {
        ...
        /*String[] textArray = splitStringByLength("abcd", 3);
        for (String s : textArray) {
            System.out.println(s);
        }*/
        System.out.println(breakLine("abcd123456", 4, 2));
    }
